...
export default function ContactFormWeb3Forms() {
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (channel === "phone" && !phone) {
      setStatus({ state: "error", message: "Hai scelto Telefono: inserisci il numero di cellulare." });
      return;
    }

    if (channel === "email" && !email) {
      setStatus({ state: "error", message: "Hai scelto Email: inserisci un indirizzo email valido." });
      return;
    }
  ...
  const inputClasses =
    "w-full rounded-xl border border-primary-foreground/15 bg-primary-foreground/5 px-4 py-3 text-sm text-primary-foreground placeholder:text-primary-foreground/30 focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent/40 transition-all duration-200";
    ...
    <form onSubmit={onSubmit} noValidate className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="flex flex-col gap-1.5">
          ...
          <input name="first_name" type="text" required autoComplete="given-name" className={inputClasses} />
        </label>
        <label className="flex flex-col gap-1.5">
          <span className={labelClasses}>Cognome *</span>
          <input name="last_name" type="text" required autoComplete="family-name" className={inputClasses} />
        </label>
      </div>
