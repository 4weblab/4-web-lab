...
/**
 * Form semplificato (2 campi) per la landing ChatGPT Ads.
 * Stilizzato per sfondi scuri, come ContactFormWeb3Forms.
 */
export default function ContactFormChatGptAds() {
  ...
  const inputClasses = (field: string) =>
    `${baseInput} ${fieldErrors[field] ? errorBorder : normalBorder}`;
  ...
  return (
    <form ref={formRef} onSubmit={onSubmit} noValidate className="space-y-4">
      <label className="flex flex-col gap-1.5">
        ...
        />
        {fieldErrors.name && <span className="text-xs text-red-400 mt-0.5">{fieldErrors.name}</span>}
      </label>

      <label className="flex flex-col gap-1.5">
        <span className={labelClasses}>Email Aziendale *</span>
        <input
          data-field="email"
          name="email"
          type="email"
        ...
        />
        {fieldErrors.email && <span className="text-xs text-red-400 mt-0.5">{fieldErrors.email}</span>}
      </label>
