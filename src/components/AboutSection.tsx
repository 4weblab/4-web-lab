      ...
      <div className="container-section">
        ...
        <div className="grid md:grid-cols-2 gap-14 items-center">
          <AnimatedSection className="space-y-6" direction="left">
            ...
            <blockquote className="body-base text-muted-foreground italic border-l-4 border-accent pl-5 py-1 bg-accent/5 rounded-r-xl">
          ...
          <StaggerContainer className="grid gap-5" staggerDelay={0.12}>
            {cards.map((card, index) => (
              <StaggerItem key={index}>
                <div className="card-glass flex items-start gap-5 group hover:-translate-y-1 transition-transform duration-300">
                  <div className="icon-box w-12 h-12 flex-shrink-0">
                    <card.icon className="w-6 h-6 text-accent-foreground" />
                  </div>
                  <div>
                    <h3 className="font-sans font-semibold text-lg mb-1.5">{card.title}</h3>
